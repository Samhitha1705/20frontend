        ...
        const fetchData = async () => {
          try {
            ...
            const [originResponse] = await Promise.all([
                axios.get(`https://webresource19.azurewebsites.net/api/v1/employeeManager/origin/${employeeId}`, {
                    method: 'GET',
                    headers: {
    ...
    useEffect(() => {
        ...
        const fetchRemainingLeaveDays = async (employeeId, leaveType, leaveStartDate, leaveEndDate) => {
            ...
            try {
                const token = localStorage.getItem('token')
                console.log(token)
                const response = await axios.get('https://webresource19.azurewebsites.net/api/leaves/remaining-leaves', {
                    method:'GET',
                    headers:{
                      'Authorization' : `Bearer ${token}`,
                      'Content-Type' : 'application/json'
...
requiredFields.forEach(field => {
    console.log(`${field}: ${formData[field]}`);
        ...
        try {
            ...
            const url = isEditing
                ? `https://webresource19.azurewebsites.net/api/leaves/update/${formData.id}`
                : `https://webresource19.azurewebsites.net/api/leaves/submit`;
