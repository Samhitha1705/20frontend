...
const EmployeeHomePage = ({ submissions, setSubmissions }) => {
  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      try {
        let url = `https://webresource19.azurewebsites.net/api/timesheets/list/${employeeId}`;
  ...
  const handleDeleteTimesheet = async () => {
    try {
      await axios.delete(`https://webresource19.azurewebsites.net/api/timesheets/delete/${selectedSubmissionId}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      });
